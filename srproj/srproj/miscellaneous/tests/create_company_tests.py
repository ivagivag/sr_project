...
class CompanyCreateViewTest(TestCase):
    ...
    def setUp(self):
        ...
        self.action = 'create company'

    def test_support_can_not_create_company_contract(self):
    ...
    def test_care_can_not_create_company_contract(self):
        self.client.login(**self.user_care_data)
        response = self.client.get(reverse(self.action))
        self.assertEqual(response.status_code, 403)
        self.assertTemplateUsed(response, 'base/403.html')

    def test_customer_can_not_create_company_contract(self):
        self.client.login(**self.user_customer1_data)
        response = self.client.get(reverse(self.action))
        self.assertEqual(response.status_code, 403)
        self.assertTemplateUsed(response, 'base/403.html')

    def test_supervisor_can_create_company_contract(self):
        self.client.login(**self.user_supervisor_data)
        response = self.client.get(reverse(self.action))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'misc/create_company_contract.html')
