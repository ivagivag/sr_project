...
class CompanyDisplayViewTest(TestCase):
    ...
    def setUp(self):
        ...
        self.action = 'display company'

    def test_support_can_not_display_company_contract(self):
    ...
    def test_care_can_not_display_company_contract(self):
        self.client.login(**self.user_care_data)
        response = self.client.get(reverse(self.action, kwargs=self.kw_client))
        self.assertEqual(response.status_code, 403)
        self.assertTemplateUsed(response, 'base/403.html')

    def test_customer_can_not_display_company_contract(self):
        self.client.login(**self.user_customer1_data)
        response = self.client.get(reverse(self.action, kwargs=self.kw_client))
        self.assertEqual(response.status_code, 403)
        self.assertTemplateUsed(response, 'base/403.html')

    def test_supervisor_can_display_company_contract(self):
        self.client.login(**self.user_supervisor_data)
        response = self.client.get(reverse(self.action, kwargs=self.kw_client))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'misc/display_company_contract.html')
