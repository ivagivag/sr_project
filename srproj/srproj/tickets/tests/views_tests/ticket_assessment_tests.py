...
from srproj.tickets.models.core_models import Ticket, TicketAssessment, TicketWorkFlow
from srproj.tickets.models.suppl_models import ProductFamily, Product

UserModel = get_user_model()


class TicketAssessViewTest(TestCase):
    product_family_data = {
    ...
    def setUp(self):
        ...
        feedback_positive_data = {
            'ticket': ticket,
            'reaction': TicketAssessment.EXCELLENT,
            'resolve': TicketAssessment.GOOD,
            'overall': TicketAssessment.VERY_GOOD,
            'creator': user_customer1
        }
        feedback_negative_data = {
            'ticket': ticket,
            'reaction': TicketAssessment.POOR,
            'resolve': TicketAssessment.FAIR,
            'overall': TicketAssessment.GOOD,
            'creator': user_customer1
        }
        feedback_positive = TicketAssessment.objects.create(**feedback_positive_data)
        feedback_negative = TicketAssessment.objects.create(**feedback_negative_data)
        self.action = 'assess ticket'
        self.kw = {'tid': ticket.id}

    def test_supervisor_can_assess_ticket(self):
        self.client.login(**self.user_supervisor_data)
        response = self.client.get(reverse(self.action, kwargs=self.kw))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'ticket_assessment/create_assessment.html')

    def test_customer_owner_can_assess_ticket(self):
        self.client.login(**self.user_customer1_data)
        response = self.client.get(reverse(self.action, kwargs=self.kw))
        self.assertEqual(response.status_code, 200)

    def test_support_assignee_can_not_assess_ticket(self):
        self.client.login(**self.user_support1_data)
        response = self.client.get(reverse(self.action, kwargs=self.kw))
        self.assertEqual(response.status_code, 403)

    def test_support_non_assignee_can_not_assess_ticket(self):
        self.client.login(**self.user_support2_data)
        response = self.client.get(reverse(self.action, kwargs=self.kw))
        self.assertEqual(response.status_code, 403)

    def test_customer_non_owner_can_not_assess_ticket(self):
        self.client.login(**self.user_customer2_data)
        response = self.client.get(reverse(self.action, kwargs=self.kw))
        self.assertEqual(response.status_code, 403)

    def test_care_can_not_assess_ticket(self):
        self.client.login(**self.user_care_data)
        response = self.client.get(reverse(self.action, kwargs=self.kw))
        self.assertEqual(response.status_code, 403)

    def test_ticket_rating_calculates_correct(self):
        ticket = Ticket.objects.get(pk=self.kw['tid'])
        self.assertEqual(ticket.rating, '9/15')

    def test_assess_ticket_fields_required(self):
        self.client.login(**self.user_customer1_data)
        response = self.client.post(reverse(self.action, kwargs=self.kw))
        for field in ('reaction', 'resolve', 'overall'):
            self.assertFormError(response, 'form', field, 'This field is required.')

    def test_user_not_logged_in_is_redirected_to_home(self):
        response = self.client.get(reverse(self.action, kwargs=self.kw))
