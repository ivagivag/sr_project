...
class TicketActiveListViewTest(TestCase):
    product_family_data = {
    ...
    def setUp(self):
        ...
        ticket_1_data = deepcopy(ticket_data)
        ticket_2_data = deepcopy(ticket_data)

        self.ticket_1 = Ticket.objects.create(**ticket_1_data)
        self.ticket_2 = Ticket.objects.create(**ticket_2_data)
        self.action = 'active tickets'

    def test_customer_can_view_ticket_active_list_and_their_content_when_creator(self):
        ...
        self.ticket_2.is_active = False
        self.ticket_2.save()
        response = self.client.get(reverse(self.action))
        self.assertEqual(len(response.context_data['object_list']), 1)

    def test_support_can_view_ticket_active_list_and_their_content_when_assignee(self):
        self.client.login(**self.user_support_data)
        response = self.client.get(reverse(self.action))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.context_data['object_list']), 2)

    def test_customer_can_view_ticket_active_list_but_only_their_own(self):
        self.client.login(**self.user_customer2_data)
        response = self.client.get(reverse(self.action))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.context_data['object_list']), 0)

    def test_care_can_view_ticket_active_list_of_all_customers(self):
        self.client.login(**self.user_care_data)
        response = self.client.get(reverse(self.action))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.context_data['object_list']), 2)

    def test_supervisor_can_view_ticket_active_list_of_all_customers(self):
