...
class TicketEnrtyCreateViewTest(TestCase):
    product_family_data = {
    ...
    def setUp(self):
        ...
        entry_data = {
            'ticket': self.ticket,
            'note': 'Some note',
            'typ': TicketWorkFlow.CORRECTIVE_ACTION,
            'creator': user_customer1
        }
        entry = TicketWorkFlow.objects.create(**entry_data)
        self.action = 'create entry'
        self.kw = {'tid': self.ticket.id}

    def test_supervisor_can_add_ticket_note(self):
        self.client.login(**self.user_supervisor_data)
        response = self.client.get(reverse(self.action, kwargs=self.kw))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'ticket_entry/create_entry.html')

    def test_customer_owner_can_add_ticket_note(self):
        self.client.login(**self.user_customer1_data)
        response = self.client.get(reverse(self.action, kwargs=self.kw))
        self.assertEqual(response.status_code, 200)

    def test_support_assignee_can_can_add_ticket_note(self):
        self.client.login(**self.user_support1_data)
        response = self.client.get(reverse(self.action, kwargs=self.kw))
        self.assertEqual(response.status_code, 200)

    def test_support_non_assignee_can_not_add_ticket_note(self):
        self.client.login(**self.user_support2_data)
        response = self.client.get(reverse(self.action, kwargs=self.kw))
        self.assertEqual(response.status_code, 403)

    def test_customer_non_owner_can_not_add_ticket_note(self):
        self.client.login(**self.user_customer2_data)
        response = self.client.get(reverse(self.action, kwargs=self.kw))
        self.assertEqual(response.status_code, 403)

    def test_care_can_not_add_ticket_note(self):
        self.client.login(**self.user_care_data)
        response = self.client.get(reverse(self.action, kwargs=self.kw))
        self.assertEqual(response.status_code, 403)

    def test_closed_ticket_can_not_be_added_entry(self):
        self.client.login(**self.user_customer1_data)
    ...
    def test_add_ticket_note_fields_required(self):
        self.client.login(**self.user_customer1_data)
        response = self.client.post(reverse(self.action, kwargs=self.kw))
        for field in ('note', 'typ'):
