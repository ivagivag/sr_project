...
class TicketAssignViewTest(TestCase):
    product_family_data = {
    ...
    def setUp(self):
        ...
        self.action = 'assign ticket'
        self.kw = {'pk': self.ticket.id}

    def test_supervisor_can_assign_ticket(self):
        self.client.login(**self.user_supervisor_data)
        response = self.client.get(reverse(self.action, kwargs=self.kw))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'tickets/assign_ticket.html')

    def test_customer_owner_can_not_assign_ticket(self):
        self.client.login(**self.user_customer1_data)
        response = self.client.get(reverse(self.action, kwargs=self.kw))
        self.assertEqual(response.status_code, 403)

    def test_support_assignee_can_not_assign_ticket(self):
        self.client.login(**self.user_support1_data)
        response = self.client.get(reverse(self.action, kwargs=self.kw))
        self.assertEqual(response.status_code, 403)

    def test_support_non_assignee_can_not_assign_ticket(self):
        self.client.login(**self.user_support2_data)
        response = self.client.get(reverse(self.action, kwargs=self.kw))
        self.assertEqual(response.status_code, 403)

    def test_customer_non_owner_can_not_assign_ticket(self):
        self.client.login(**self.user_customer2_data)
        response = self.client.get(reverse(self.action, kwargs=self.kw))
        self.assertEqual(response.status_code, 403)

    def test_care_can_not_assign_ticket(self):
        self.client.login(**self.user_care_data)
        response = self.client.get(reverse(self.action, kwargs=self.kw))
        self.assertEqual(response.status_code, 403)

    def test_closed_ticket_can_not_be_assigned(self):
        self.client.login(**self.user_supervisor_data)
