...
class TicketDisplayViewTest(TestCase):
    product_family_data = {
    ...
    def setUp(self):
        ...
        self.user_support = UserModel.objects.create_user(**self.user_support_data)
        ...
        group_support.user_set.add(self.user_support)
        ...
        self.action = 'ticket details'
        self.kw = {'pk': self.ticket.id}

    def test_support_can_not_view_ticket(self):
        self.client.login(**self.user_support_data)
        response = self.client.get(reverse(self.action, kwargs=self.kw))
        self.assertEqual(response.status_code, 403)

    def test_customer_not_owner_can_not_view_ticket(self):
        self.client.login(**self.user_customer2_data)
        response = self.client.get(reverse(self.action, kwargs=self.kw))
        self.assertEqual(response.status_code, 403)

    def test_care_can_view_ticket(self):
        self.client.login(**self.user_care_data)
        response = self.client.get(reverse(self.action, kwargs=self.kw))
        self.assertEqual(response.status_code, 200)

    def test_supervisor_can_view_ticket(self):
        self.client.login(**self.user_supervisor_data)
        response = self.client.get(reverse(self.action, kwargs=self.kw))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'tickets/display_ticket.html')

    def test_customer_owner_can_view_ticket(self):
        self.client.login(**self.user_customer1_data)
        response = self.client.get(reverse(self.action, kwargs=self.kw))
        self.assertEqual(response.status_code, 200)

    def test_support_can_view_ticket_when_becomes_assignee(self):
        self.ticket.assignee = self.user_support
        self.ticket.save()
        self.client.login(**self.user_support_data)
        response = self.client.get(reverse(self.action, kwargs=self.kw))
        self.assertEqual(response.status_code, 200)
